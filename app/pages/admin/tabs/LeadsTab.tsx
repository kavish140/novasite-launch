...
export default function LeadsTab({
  requests, loading, onUpdateStatus, onBulkResolve, onBulkDelete,
}: LeadsTabProps) {
  ...
  const exportSelected = () => {
    ...
    a.download = `audit-leads-selected-${format(new Date(), "yyyy-MM-dd")}.csv`;
    a.click();
  ...
  const renderTable = (rows: AuditRequest[], allRows: AuditRequest[], isPending: boolean) => {
        ...
        <Table>
          ...
          <TableBody>
            {rows.map((request, i) => (
              <TableRow
                key={request.id}
                className={`cursor-pointer hover:bg-primary/5 transition-colors ${i % 2 === 0 ? "bg-background/20" : ""} ${selectedIds.has(request.id) ? "bg-primary/5" : ""}`}
    ...
    <motion.div className="space-y-6" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}>
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <Input
            placeholder="Search name, email, mobile, website…"
